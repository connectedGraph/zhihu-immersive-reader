...
- 4 套内置主题配色：宣纸、蓝白、吸血鬼、暗蓝，日间与夜间主题各两套；自定义主题仍可单独添加
...
- 支持每组 6 篇的手动分组翻页，或三组后台预取的连续滚动；连续模式在最后一组下方提供可回弹加载门，继续滚动后模拟加载并平滑切换至下一批
- 推荐与关注动态都支持批次分隔、轻度滚动吸附和向上回看；网络跟不上时会先消费预取队列，而不是重复并发请求
...
- 通过知乎回答 Feed API (`/api/v4/questions/{question_id}/feeds`) 分批加载回答，每批 6 条，并沿 `paging.next` 继续翻页
- 回答卡片提供与首页推荐流一致的互动栏：赞同、反对、感谢、收藏、评论跳转和打开原文
- 每次进入问题页重新获取回答列表，不依赖回答采集缓存或旧的“已采集完”状态
- API 不可用时只读取当前页面已经渲染的回答，不再通过反复触底滚动驱动知乎页面加载
- 插件不会主动发送知乎 `/lastread/touch` 已读请求
...
- **个性化**：默认进入沉浸模式、首页推荐分组/连续滚动、目标翻译语言、翻译提示词库、上下文段落、自动摘要、自动翻译、图片自动隐藏、分享格式
...
- **feat**: 个性化设置新增「默认进入沉浸模式」开关（默认开启），关闭后打开知乎页面保持原样、不再自动进入沉浸；仍可随时按 Ctrl+E 手动展卷
- **refactor**: 问答页回答采集改用知乎回答 Feed API (`/api/v4/questions/{question_id}/feeds`)，按每批 6 条加载并跟随 `paging.next` 翻页，不再依赖重复滚动采集回答
- **feat**: 问答页 API 回答卡片增加与首页推荐流一致的互动栏，支持赞同、反对、感谢、收藏、评论跳转和打开原文
- **fix**: 移除问答页回答采集缓存及其 `exhausted` 状态持久化，进入问题页时重新获取首批回答，避免旧缓存导致后续加载被永久关闭
- **fix**: API 失败时的问答页 fallback 改为只读取当前已经渲染的回答，不再触底滚动或为了凑够数量继续驱动知乎加载
- **fix**: 问答页互动操作不再错误写入首页或回答缓存；当前互动状态仅在本次问答页会话中保留
- **chore**: 问答页回答采集不主动发送知乎 `/lastread/touch` 请求，已读状态不由插件主动同步

### 2026-08-05 · v5.1.5

- **feat**: 新增回到顶部按钮，悬浮文章右下角（工具面板上方），样式与复制 Markdown 按钮一致、尺寸同侧边栏按钮，受 Ctrl+H 隐藏控制
- **style**: 进入沉浸模式改为 0.25s 淡入，与退出淡出对称；移除 `#immersive-wrapper` 的 `transition: all` 拖拽动画，仅保留 opacity 与背景色过渡，主题切换过渡不受影响
- **fix**: 修复专栏页退出沉浸模式后顶部导航栏/搜索框残留隐藏——React 重渲染会重置元素 className、抹掉手动添加的隐藏标记，导致退出恢复逻辑遗漏；改为依赖样式表隐藏，并在退出时按选择器显式兜底恢复

### 2026-07-28 · v5.1.4

- **fix**: 修复专栏文章沉浸模式中评论按钮无法展示评论的问题；保留旧版 `article → ContentItem-actions` 结构，只有点击评论按钮后才将动态生成的 `.Comments-container` 挂载到操作栏下方并平滑定位
- **fix**: 评论按钮采用附加事件绑定，不阻断知乎原有 React 点击逻辑；评论尚未生成时保留用户请求，待 CommentList 动态挂载后再显示，退出沉浸模式时清理 Observer 和监听器
- **style**: `.CommentContent`、评论卡片、评论链接与输入区纳入主题变量，跟随正文文字、纸张、强调色和边框配色切换

### 2026-07-22 · v5.1.3

- **feat**: 末组加载门改为显示 0–100% 的圆环进度，累计距离调整为半个视窗高度，中途停止会回弹归零，到达 100% 后平滑切换至下一批
- **fix**: 滚动吸附改为首次真实滚轮或触摸滚动后启用，标题栏、首张卡片和后续批次仍保留吸附点，避免首屏尚未滚动便自动定位
- **style**: 内置主题调整为宣纸、蓝白、吸血鬼、暗蓝两套日间与两套夜间配色，降低蓝白主题亮度和吸血鬼主题的黑红对比，并兼容旧主题索引迁移

### 2026-07-22 · v5.1.2

- **feat**: 首页推荐与关注动态新增独立的三组预取队列，滚动和手动加载统一从队列取数，API 请求串行补位
- **feat**: 连续滚动改为末组底部可回弹加载门，中途停止会收回留白，继续滚动则经过 0.4–0.7 秒模拟加载后平滑切换至下一批
- **feat**: 各批次起点增加轻度滚动吸附；快速消费完三组缓存时显示网络等待状态，失败后可继续滚动重试
